...
Segmentor::Segmentor(const std::string& name, const BT::NodeConfiguration& config)
...
BT::NodeStatus Segmentor::onStart()
{
  ...
  segmentor_ = std::make_unique<ros2_yolos_cpp::SegmentorAdapter>();
  if (!segmentor_->initialize(c))
  {
  ...
  image_subscriber = node_raw_ptr->create_subscription<sensor_msgs::msg::Image>(
      image_topic, 1, std::bind(&Segmentor::imageCB, this, std::placeholders::_1));
...
void Segmentor::unsubscribeTopics()
{
  image_subscriber.reset();
  debug_publisher.reset();
}

void Segmentor::imageCB(const sensor_msgs::msg::Image::SharedPtr msg)
{
