 ...
 *
 */
class RemoveCollisionObjects : public BT::SyncActionNode
{
public:
  /**
   * @brief Constructor for RemoveCollisionObjects behavior
   *
   * @param name Name of behavior
   * @param config BTCPP node config
   */
  RemoveCollisionObjects(const std::string& name, const BT::NodeConfig& config);
