...
static const char* xml_text = R"(
 <root BTCPP_format="4">
     <BehaviorTree ID="MoveitTestTree">
        <Sequence>
            <ExecuteTrajectory name="sleepA" msec="2000"/>
            <PlanToJointState name="sleepA" msec="2000"/>
        </Sequence>
     </BehaviorTree>
 </root>
 )";
...
protected:
  void SetUp() override
  {
    ...
    tree_exec_server->node()->set_parameters(std::vector<rclcpp::Parameter>{
        rclcpp::Parameter("plugins", std::vector<std::string>{ "moveit_behavior/bt_plugins" }) });
...
TEST_F(BehaviorPluginTest, TestBehaviorRegistration)
{
  ...
  EXPECT_THAT(tree_exec_server->factory().registeredBehaviorTrees(), ::testing::Contains("MoveitTestTree"));
}
