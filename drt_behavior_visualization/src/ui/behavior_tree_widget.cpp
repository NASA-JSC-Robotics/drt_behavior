...
void BehaviorTreeWidget::on_UpdateTreesBtn_pressed()
{
  ...
  auto future = get_behavior_trees_client_->async_send_request(
      request, std::bind(&BehaviorTreeWidget::get_behavior_trees_cb, this, std::placeholders::_1));
  return;
}
...
void BehaviorTreeWidget::on_RunButton_pressed()
{
  ...
  send_goal_options.result_callback = std::bind(&BehaviorTreeWidget::result_callback, this, std::placeholders::_1);
...
{
  std::string result;
  result.reserve(text.size() * 2);  // Pre-allocate memory to prevent reallocations
  ...
  while (i < text.size())
  {
    ...
    {
      // Check if the current substring matches our token
      if (text.compare(i, old_text.size(), old_text) == 0)
      {
        result.append(new_text);
        i += old_text.size();
        matched = true;
        break;  // Move to the next position in the main string
      }
    }
  ...
  text = std::move(result);  // Efficiently swap the processed string back
}
